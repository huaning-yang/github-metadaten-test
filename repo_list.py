...
for i in range(0,len(json)):
  ...
  collaborators = requests.get('https://api.github.com/repos/'+username+'/'+repo+'/collaborators')
  json3 = collaborators.json()
  ...
  if(collaborators.status_code == 200):
    mdFile.new_line('Collaborators: ')
    for j in range(0,len(json3)):
      mdFile.new_inline_link(link=json3[j]['html_url'], text=json3[j]['login'])
